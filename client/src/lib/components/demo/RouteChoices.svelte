...
<div class="route-results-content space-y-2">
	{#each entries as result, i (result.road.key)}
		<button
			class="demo-route-card btn h-auto w-full justify-start gap-3 p-3 text-left"
		...
		>
			...
			<span class="route-card-main flex min-w-0 flex-1 flex-col gap-1"
				><strong
					...
					<small>{(result.road.distanceMeters / 1000).toFixed(1)} km</small></strong
				>
				<span
				...
				>
				<small class:text-error={result.blocked}
					>{result.blocked
						? 'Blocked by simulated flood'
						: floodSimulation
							? 'No blocking simulated flood'
							: 'Flood conditions unconfirmed'}</small
				>
			</span><span class="route-radio status status-primary"></span>
		</button>
	{/each}
