...
export function intersectsFlood(path: Coordinate[], zone: FloodHazardZone): boolean {
	if (!zone.active || zone.depthCm <= 0) return false;
	const project = ([lat, lng]: Coordinate) => [
		(lng - zone.center[1]) * 111320 * Math.cos((zone.center[0] * Math.PI) / 180),
...
export function cumulativeDistances(path: Coordinate[]): number[] {
	...
	for (let i = 1; i < path.length; i++)
		result.push(result[i - 1] + haversineDistanceKm(path[i - 1], path[i]) * 1000);
	return result;
}
...
export function positionAt(path: Coordinate[], meters: number): Coordinate {
	...
	for (let i = 1; i < path.length; i++) {
		if (distances[i] >= meters && distances[i] > distances[i - 1]) {
			const t = Math.max(0, (meters - distances[i - 1]) / (distances[i] - distances[i - 1]));
			return [
...
export function parseRoadRoutes(data: unknown, source: RoadRoute['source']): RoadRoute[] {
	const response = data as { code?: string; routes?: unknown[] };
	if (response?.code !== 'Ok' || !Array.isArray(response.routes) || !response.routes.length)
	...
	return response.routes.map((raw, index) => {
		...
		const polyline: Coordinate[] = route.geometry.coordinates.map((p) => {
			if (p.length < 2 || !p.every(Number.isFinite) || Math.abs(p[0]) > 180 || Math.abs(p[1]) > 90)
		...
		const steps: NavigationStep[] = route.legs
			...
			.map((s, i) => {
				...
				const coordinate: Coordinate = [s.maneuver.location[1], s.maneuver.location[0]];
				...
				const action =
					maneuver === 'arrive'
						...
						: s.maneuver.type === 'depart'
							? 'Depart'
							: s.maneuver.type.includes('roundabout') || s.maneuver.type === 'rotary'
								...
								: maneuver === 'u-turn'
									...
									: maneuver.includes('left')
										...
										: maneuver.includes('right')
											? 'Turn right'
											: 'Continue';
				const instruction = maneuver === 'arrive' ? action : `${action} on ${street}`;
				return {
					id: `step_${i}`,
					instruction,
					cebuanoInstruction:
...
export function evaluateRoutes(
	...
	const evaluate = (road: RoadRoute, id: RouteOption['id']): RouteOption => {
		...
		return {
			id,
			source: road.source,
			name: id === 'primary' ? 'Primary road route' : 'Flood-avoiding alternative',
	...
	const alternativeSafe =
		roads
			.slice(1)
			.filter((r) => JSON.stringify(r.polyline) !== JSON.stringify(roads[0].polyline))
