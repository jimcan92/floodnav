	...
	return data.map((r: SensorReading) => {
		if (
			...
			(r.water_depth_cm !== null &&
				(!Number.isFinite(r.water_depth_cm) || r.water_depth_cm < 0 || r.water_depth_cm > 1000)) ||
...
export function sensorIsFresh(r: SensorReading, now = Date.now()): boolean {
	if (r.water_depth_cm === null || !r.observed_at || !r.received_at) return false;
...
export async function fetchSensorReadings(
	...
	if (rows.length >= 1000)
		throw new Error('Sensor limit reached; pagination is required before using this feed.');
	return rows;
}
