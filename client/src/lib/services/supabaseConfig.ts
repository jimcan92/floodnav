...
export function validateSupabaseConfig(config: SupabaseConfig): SupabaseConfig {
	...
	if (
		...
		throw new Error('Use the HTTPS project URL only, without an API path or credentials.');
	if (key.startsWith('sb_secret_'))
		throw new Error('Use a publishable key, never a secret key in the frontend.');
	if (!key.startsWith('sb_publishable_')) {
		try {
			const payload = JSON.parse(atob(key.split('.')[1].replace(/-/g, '+').replace(/_/g, '/')));
			if (!key.startsWith('eyJ') || payload.role !== 'anon') throw new Error();
		} catch {
			throw new Error('Only a publishable or legacy anon key belongs in the frontend.');
		}
	}
...
export function loadSupabaseConfig(
	fallback: SupabaseConfig = { url: '', key: '' }
): SupabaseConfig {
	try {
...
export function saveSupabaseConfig(config: SupabaseConfig): void {
	localStorage.setItem(storageKey, JSON.stringify(validateSupabaseConfig(config)));
}
