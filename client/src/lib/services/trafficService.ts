import type { Coordinate, TrafficDot, TrafficLevel } from '../types/navigation';
...
export function haversineDistanceKm(coord1: Coordinate, coord2: Coordinate): number {
	...
	for (let i = 0; i < polyline.length - 1; i++) {
		...
		for (let j = 0; j < numPoints; j++) {
			...
			const matchedZone = knownTrafficZones.find(
				(zone) => haversineDistanceKm(point, zone.centerCoordinate) <= zone.radiusKm
			);
