...
import { Coordinate, DemoScenario, FloodHazardZone } from "../types/navigation";
...
if (!primaryOnly)
  throw new Error("Demo fixture requires a separate bypass corridor.");
export function scenarioFloods(scenario: DemoScenario): FloodHazardZone[] {
  ...
  return [
    ...
    zone(
      DEMO_ROADS[1].polyline[Math.floor(DEMO_ROADS[1].polyline.length / 2)],
      "bypass-flood",
    ),
  ];
}
